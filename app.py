...
st.set_page_config(page_title="Spam Classifier", page_icon="📧", layout="wide", initial_sidebar_state="collapsed")
 
st.markdown("""
<style>
    .main-header { background: linear-gradient(135deg, #6366F1, #4F46E5); padding: 2rem; border-radius: 12px; text-align: center; color: white; margin-bottom: 2rem; }
    .main-header h1 { font-size: 2.2rem; margin: 0; }
    .main-header p  { font-size: 1rem; opacity: 0.9; margin: 0.5rem 0 0; }
    .metric-card { background: white; border-radius: 10px; padding: 1.2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.08); text-align: center; border-left: 4px solid #6366F1; }
    .metric-card h3 { font-size: 1.8rem; color: #6366F1; margin: 0; }
    .metric-card p  { color: #64748B; margin: 0; font-size: 0.9rem; }
    .result-spam { background: linear-gradient(135deg, #FEE2E2, #FECACA); border: 2px solid #DC2626; border-radius: 12px; padding: 2rem; text-align: center; margin: 1rem 0; }
    .result-ham  { background: linear-gradient(135deg, #D1FAE5, #A7F3D0); border: 2px solid #059669; border-radius: 12px; padding: 2rem; text-align: center; margin: 1rem 0; }
...
def has_url(text):
    return bool(re.search(r'http|www|\.com|\.net|\.org|bit\.ly', text, re.IGNORECASE))
...
<div class="main-header">
    ...
    <p>AI-powered spam detection — SVM + TF-IDF with bigrams | 98.45% Accuracy</p>
...
with c2: st.markdown(f'<div class="metric-card"><h3>{metrics.get("roc_auc","0.9920")}</h3><p>ROC-AUC</p></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with col_examples:
        ...
        examples = {
            ...
            "🚨 Spam 3": "Congrats! 1 year special cinema pass for 2 is yours. call 09061209465 now! All 4 FREE!",
    ...
    with col_main:
        ...
        message = st.text_area("Paste or type your email/SMS message here:", value=default_msg, height=160, placeholder="Type or paste a message here...")
    ...
    if classify_btn and message.strip():
        ...
        with res_col:
            if pred == 1:
                st.markdown(f"""<div class="result-spam"><h1>🚨 SPAM</h1><h2 style="color:#DC2626;font-size:2.5rem;margin:0">{spam_pct:.1f}%</h2><p style="color:#7F1D1D;">Probability of being Spam</p></div>""", unsafe_allow_html=True)
            else:
                st.markdown(f"""<div class="result-ham"><h1>✅ HAM (Legitimate)</h1><h2 style="color:#059669;font-size:2.5rem;margin:0">{ham_pct:.1f}%</h2><p style="color:#064E3B;">Probability of being Legitimate</p></div>""", unsafe_allow_html=True)
            ...
            if spam_pct > 10:
                ax_b.text(spam_pct/2, 0, f"{spam_pct:.1f}%", ha='center', va='center', color='white', fontweight='bold', fontsize=10)
            if ham_pct > 10:
                ax_b.text(spam_pct + ham_pct/2, 0, f"{ham_pct:.1f}%", ha='center', va='center', color='white', fontweight='bold', fontsize=10)
            plt.tight_layout()
            st.pyplot(fig_b)
            plt.close()
        ...
        with detail_col:
            ...
            url_found   = has_url(message)
            phone_found = has_phone(message)
            up_ratio    = sum(c.isupper() for c in message) / max(len(message), 1) * 100
 
            st.markdown(f"**Original length:** {len(message)} characters")
            st.markdown(f"**Word count:** {len(message.split())} words")
            st.markdown(f"**Has URL:** {'Yes ⚠️' if url_found else 'No ✅'}")
            st.markdown(f"**Has phone/shortcode number:** {'Yes ⚠️' if phone_found else 'No ✅'}")
            st.markdown(f"**Uppercase ratio:** {up_ratio:.1f}%")
            st.markdown(f"**Exclamation marks:** {message.count('!')}")
            st.markdown(f"**Special chars (!, ?, $, £):** {len(re.findall(r'[!?$£€]', message))}")
...
with tab2:
    ...
    c1, c2 = st.columns(2)
    with c1:
        for img, cap in [("model_comparison.png","Model Comparison"), ("confusion_matrix.png","Confusion Matrix")]:
            path = os.path.join(OUTPUT_DIR, img)
            if os.path.exists(path): st.image(path, caption=cap, use_container_width=True)
    with c2:
        for img, cap in [("roc_curves.png","ROC Curves"), ("spam_keywords.png","Top Spam Keywords")]:
            path = os.path.join(OUTPUT_DIR, img)
            if os.path.exists(path): st.image(path, caption=cap, use_container_width=True)
    st.markdown("---")
    c3, c4 = st.columns(2)
    with c3:
        ...
        if os.path.exists(path): st.image(path, caption="Word Clouds — Ham vs Spam", use_container_width=True)
    with c4:
        path = os.path.join(OUTPUT_DIR, "length_distribution.png")
        if os.path.exists(path): st.image(path, caption="Message Length Distribution", use_container_width=True)
    st.markdown("---")
...
with tab3:
    ...
    with c1:
        st.markdown("""
        ### 🔬 About This Project
        Advanced NLP spam detection trained on 5,157 real SMS/email messages.
